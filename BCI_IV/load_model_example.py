...
print("SUBJECT", subject_id, "ARCHITECTURE: Baseline")
print("LOAD PARAMETERS FOR SUBJECT ", subject_id)

clf2.load_params(f_params='High_freq_baseline_arch_kostas_model1.pkl')
...
y_test_list = []
y_true_list = []
for X, y, i in clf2.get_iterator(valid_set, drop_index=False):
    #print(X[0])
    y_test_list.append(clf2.predict(X))
    y_true_list.append(y.detach().numpy())

from sklearn import metrics
acc_list = []
for i in range(len(y_true_list)):
    acc_list.append(metrics.accuracy_score(y_true_list[i], y_test_list[i]))

def Average(lst):
    return sum(lst) / len(lst)

average = Average(acc_list)
print("FINAL ACCURACY OF SUBJECT ", subject_id)
print(average)

laod_saved_model = kostas(
    n_chans,
    n_classes,
    input_window_samples=input_window_samples,
    final_conv_length=30,
)
laod_saved_model.load_state_dict(torch.load('High_freq_baseline_arch_kostas_model1.pkl'))
