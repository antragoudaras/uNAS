...
for i in range(len(train_set)):
    x_train_list.append(train_set[i][0])
    y_train_list.append(train_set[i][1])

for i in range(len(valid_set)):
    x_valid_list.append(valid_set[i][0])
    y_valid_list.append(valid_set[i][1])

x_train = np.expand_dims(x_train_list, axis=-1)
...
# x_train = np.reshape(x_train, (len(train_set), 22, 1125))
# x_train = np.array(train_list, dtype=object)[:, 0]
# y_train = np.array(train_list, dtype=np.float32)[:, 1]
